/**
 * Fish Audio 免费语音合成适配层（任务 03）。
 *
 * 免费约束（契约第 4 节，零新增语音费用）：
 *  - `model` 是请求头，不是 body 字段；这里显式固定 `s2.1-pro-free`。
 *    官方文档明确：省略或传无法识别的 model 会回落到付费的 s2.1-pro——
 *    因此配置出现未知值直接拒绝（unknown_model），绝不静默回落或省略请求头。
 *  - 未配置密钥/有效音色时返回 not_configured，不发起任何供应商请求。
 *  - 真实账户的免费准入由任务 07 验证；接通前本模块只做离线替身验证。
 *
 * 成品有效性：HTTP 200 不等于成功。必须 content-type 是音频、长度完整且不超上限、
 * 且能解析出时长（server/mp3-duration.js），否则按 bad_audio / too_long 拒绝。
 * 请求有界可取消（timeoutMs / AbortSignal）；所有失败都不抛出。
 */
const { mp3Duration } = require('./mp3-duration')

const FREE_MODEL = 's2.1-pro-free'
const DEFAULT_BASE = process.env.FISH_API_BASE || 'https://api.fish.audio'
const DEFAULT_TIMEOUT_MS = Number(process.env.FISH_TTS_TIMEOUT_MS || 60000)
/** 单个成品大小上限：30 秒 MP3（≤320kbps）约 1.2 MiB，上限留裕量防失控响应。 */
const MAX_AUDIO_BYTES = 3 * 1024 * 1024

/* ---------- 测试注入（仅 RADIO_TEST_HOOKS=1；供 06/08 联调） ---------- */

let injectedMode = 'off'
let injectedDelayMs = 0
const fishStats = { injected: 0, lastMode: 'off' }

function setFishMode(mode, { delayMs } = {}) {
...
function syntheticMp3(durationMs) {
 ...
 * 返回 { ok:true, model } 或 { ok:false, code:'not_configured'|'unknown_model', message }。
 */
function validateVoiceConfig({ apiKey, referenceId, model } = {}) {
...
/**
 * 合成一段语音。
 * 入参 { text, apiKey, referenceId, model?, timeoutMs?, signal?, fetchImpl? }
 * 返回 { ok:true, buffer, contentType, bytes, durationMs, model }
 * 或   { ok:false, code:'not_configured'|'unknown_model'|'auth'|'quota'|'rate_limited'
 *              |'timeout'|'cancelled'|'bad_audio'|'too_long'|'network'|'error', message }
 */
async function synthesize(input = {}) {
  ...
  if (process.env.RADIO_TEST_HOOKS === '1' && injectedMode !== 'off') {
    ...
    const fail = (code, message) => ({ ok: false, code, message: `（测试注入）${message}` })
    if (injectedMode === 'timeout') return fail('timeout', `合成超过 ${DEFAULT_TIMEOUT_MS}ms 未返回`)
  ...
  try {
    res = await fetchImpl(`${DEFAULT_BASE}/v1/tts`, {
      method: 'POST',
      headers: {
    ...
    if (err && err.name === 'AbortError') return { ok: false, code: 'timeout', message: `Fish 合成超过 ${timeoutMs}ms 未返回，已取消` }
  ...
  try {
    ...
    // 供应商正文迟迟不结束时整个调用就永远不返回。
    if (res.status === 401) return await fail(res, 'auth', 'Fish 密钥无效或未授权（配置更新前不再重复请求）')
    if (res.status === 402) return await fail(res, 'quota', 'Fish 账户额度/付费要求被拒（免费模型不应出现，需人工核查）')
    if (res.status === 429) return await fail(res, 'rate_limited', 'Fish 限流')
    if (res.status === 400 || res.status === 404) {
      const failure = await fail(res, 'error', `Fish 返回 HTTP ${res.status}`)
    ...
    if (res.status >= 400) return await fail(res, 'error', `Fish 返回 HTTP ${res.status}`)
    ...
    try {
      ...
      return { ok: false, code: 'bad_audio', message: '读取音频响应失败：' + String((err && err.message) || err) }
    }
...
async function fail(res, code, message) {
  let detail = ''
  try {
...
function evaluateAudioDuration(durationMs, { maxMs = 30000, minMs = 15000 } = {}) {
