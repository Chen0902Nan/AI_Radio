...
export interface LibraryReport {
  blocked?: string
  liked?: { idsReturned: number; idsUnique: number; tracksReturned: number; missingDetails: number; [key: string]: unknown }
  pagination?: { consistent: boolean; [key: string]: unknown }
  playlistTracks?: Array<{ id: number; name: string; error?: string; complete?: boolean | null; returned?: number; unique?: number; declaredTrackCount?: number; [key: string]: unknown }>
  playability?: { distribution?: { full?: number; trial?: number; none?: number; error?: number }; [key: string]: unknown }
  steps?: Array<{ step: string; ok: boolean; error?: string; [key: string]: unknown }>
}
export function collectFailures(report: LibraryReport): string[] {
  const failures = []
  ...
  if (report.playability) {
    ...
    if ((d.error ?? 0) > 0) failures.push(`音源查询报错 ${d.error} 首`)
