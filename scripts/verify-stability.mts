 ...
 *
 * 用法：node scripts/verify-stability.mts [--base=http://127.0.0.1:8792] [--duration-min=120]
 */
import fs from 'node:fs'
import path from 'node:path'
import os from 'node:os'
import puppeteer from 'puppeteer-core'
import { isPlaybackStalled } from './lib/playback-checks.mts'
...
const log = (...a: string[]) => console.log(new Date().toISOString().slice(11, 19), ...a)
...
async function main() {
  ...
  page.on('pageerror', (e) => report.errors.push({ at: Date.now(), kind: 'pageerror', message: String((e instanceof Error ? e.message : String(e))).slice(0, 200) }))
  await page.exposeFunction('recordRadioMediaEvent', (event: any) => report.events.push({ at: Date.now(), ...event }))
      ...
      try {
        const s = await page.evaluate(() => {
          ...
          return {
            ...
            radio: JSON.parse(document.querySelector<HTMLElement>('[data-radio-observation]')?.getAttribute('data-radio-observation') || 'null'),
          }
        })
        const health = await fetch(BASE + '/api/health').then((r) => r.json()).catch((e) => ({ ok: false, err: String((e instanceof Error ? e.message : String(e))) }))
        ...
        report.errors.push({ at: Date.now(), kind: 'sample', message: String((err instanceof Error ? err.message : String(err))).slice(0, 200) })
        log('采样失败:', (err instanceof Error ? err.message : String(err)))
      }
    }
...
main().catch((e) => {
  console.error('稳定性观测失败:', (e instanceof Error ? e.message : String(e)))
  process.exit(1)
})
