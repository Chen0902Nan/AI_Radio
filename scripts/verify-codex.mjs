 ...
 * 用法：node scripts/verify-codex.mjs [--headful] [--skip-real] [--base=http://127.0.0.1:8787]
 */
...
async function runRealCall() {
  ...
  const allFromCandidates = ids.every((id) => candidateIds.has(id))
  ...
  check('Codex 返回的每一首都有中文选歌理由', r.picks.every((p) => p.reason && p.reason.length > 0), {
    reasons: r.picks.map((p) => p.reason),
  })
  check('Codex 返回值用真实账号做过可播性校验', r.picks.every((p) => p.playable === true), {
    kinds: r.picks.map((p) => p.kind),
    dropped: r.dropped || [],
  })
  report.evidence.realCall = {
    brief: '深夜安静、适合一个人听',
    picks: r.picks.map((p) => ({ id: p.id, name: p.name, artists: p.artists, reason: p.reason, kind: p.kind })),
...
async function runBrowserChecks() {
  ...
  check('界面上能看到 Codex 的选歌理由', afterPlan.codexPicks.every((p) => p.reason && p.reason.length > 0), {
