 ...
 *
 * 用法：node scripts/openapi-probe.mjs [--song=65536]
 */
...
async function callOpenApi(pathname, bizContent, { accessToken = null, method = 'POST' } = {}) {
  const params = {
    appId: cred.appId,
...
function maskItem(item) {
  ...
  for (const [k, v] of Object.entries(item)) {
    ...
    else if (k === 'url') out[k] = v ? `<有地址，长度 ${String(v).length}，host=${safeHost(v)}>` : null
