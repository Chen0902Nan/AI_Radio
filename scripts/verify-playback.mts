 ...
 * 用法：node scripts/verify-playback.mts [--headful] [--base=http://127.0.0.1:8787]
 *
...
const report: ProbeReport & { checks: Array<{name: string; ok: unknown; detail?: unknown}>; evidence: Record<string, any> } = { at: new Date().toISOString(), base: BASE, headless: HEADLESS, checks: [], evidence: {} }
...
function skipCheck(name: string, detail: { why: string }) {
...
async function waitFor<T>(fn: () => T | Promise<T>, { timeout = 30000, interval = 500, label = 'condition' } = {}) {
...
async function main() {
  ...
  const trace: Array<{ at: number; kind: string; status?: number; url?: string; [key: string]: unknown }> = []
  page.on('request', (req) => {
    const u = req.url()
  ...
  const srcMatches = (s: { audioSrc: string|string[] }, id: any) => Boolean(s && id && s.audioSrc.includes(`/api/audio/${id}?`))
  const decoded = () =>
    page.evaluate(() => {
  ...
  while (Date.now() < deadline) {
    ...
    endedEvent = ev.find((e: { type: string; id: any }) => e.type === 'ended' && e.id === restart.id) || null
