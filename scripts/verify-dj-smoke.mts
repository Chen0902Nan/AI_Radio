 ...
 * 用法：node scripts/verify-dj-smoke.mts [--headful] [--base=http://127.0.0.1:8787]
 *
...
const report: ProbeReport & { checks: Array<{name: string; ok: unknown; detail?: unknown}> } = { at: new Date().toISOString(), base: BASE, headless: HEADLESS, checks: [] }
...
const trackArgs = (tracks: any[]) =>
  tracks.map((t: { id: any; name: any; artists: any; album: any; durationMs: any }) => ({ id: t.id, name: t.name, artists: t.artists, album: t.album, durationMs: t.durationMs }))
const radioState = (page: Page) => page.evaluate(() => window.__radio.state)

function playTrack(page: import('puppeteer-core').Page, index: number) {
  return page.evaluate((i: number) => document.querySelectorAll<HTMLElement>('.track')[i].click(), index)
}
function seekEnd(page: import('puppeteer-core').Page) {
  return page.evaluate(() => window.__radio.__test.seekToEnd())
}
function waitTrackPlaying(page: import('puppeteer-core').Page, index: number) {
  return waitFor(
    () => page.evaluate((i: any) => {
...
function setDjEnabled(page: import('puppeteer-core').Page, checked: boolean) {
  return page.evaluate((v: any) => {
    const box = document.querySelector<HTMLInputElement>('#djEnabled')
    box!.checked = v
 ...
 */
async function driveToSegue(page: import('puppeteer-core').Page, tracks: any[]) {
  await page.evaluate((ts: any) => {
...
async function main() {
  ...
  await page.evaluate((tracks) => {
    ...
    document.querySelector<HTMLInputElement>('#djEnabled')!.checked = true
    document.querySelector<HTMLInputElement>('#djEnabled')!.dispatchEvent(new Event('change'))
  ...
  const segueState = await page.evaluate(() => ({
    ...
    likeDisabled: document.querySelector<HTMLButtonElement>('#like')!.disabled,
  }))
  ...
  async function scenario(name: string, fn: { (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): Promise<void>; (): any }) {
    log(`\n[场景] ${name}`)
    try {
      await fn()
    } catch (err) {
      check(`${name} —— 场景中断`, false, { error: (err instanceof Error ? err.message : String(err)) })
    }
  }
  ...
  await scenario('复核 4 · 停止后重开', async () => {
    ...
    const restarted = await page.evaluate(() => ({
      ...
      stopDisabled: document.querySelector<HTMLButtonElement>('#stop')!.disabled,
    }))
  ...
  await scenario('复核 3 · 迟到试听', async () => {
    ...
    await page.evaluate((ref) => {
      document.querySelector<HTMLInputElement>('#previewVoice')!.value = ref
  ...
  await scenario('复核二 3 · 重播同一首歌', async () => {
    ...
    await page.evaluate((i) => {
      ...
      document.querySelectorAll<HTMLElement>('.track')[i].click()
      document.getElementById('audio')!.dispatchEvent(new Event('ended'))
    }, beforeReplay.index)
    await sleep(2000)
