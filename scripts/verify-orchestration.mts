import type { ProbeReport, NaturalPlaybackTrace } from './lib/tool-types.d.ts'
/**
 * 自动补歌与统一节目编排的验证。
 *
 * 覆盖本次范围里可观察的行为：
 ...
 *
 * 用法：node scripts/verify-orchestration.mts [--skip-real] [--server-only] [--headful] [--base=http://127.0.0.1:8792]
 */
...
import { isBatchTrackPlaying } from './lib/playback-checks.mts'
...
const report: ProbeReport & { checks: Array<{name: string; ok: unknown; detail?: unknown}>; evidence: Record<string, any> } = { at: new Date().toISOString(), base: BASE, isolated: !EXTERNAL_BASE, checks: [], evidence: {} }
...
async function waitFor<T>(fn: () => T | Promise<T>, { timeout = 30000, interval = 300, label = 'condition' } = {}) {
...
async function api(pathname: string, options: { method?: string; body?: unknown } = {}): Promise<Record<string, any> & {status: number}> {
...
const post = (p: string, body?: Record<string, unknown>) => api(p, { method: 'POST', body: body || {} })
...
let reportBrowser: Browser|null = null
...
async function runControllerUnitChecks() {
  ...
  const calls: any[] = []
  const batches: any[] = []
  const statuses: { text: any; cls: any }[] = []
  let responder: (request?: unknown) => Promise<{ok: boolean; picks?: Array<{id: number; type: string}>; code?: string; message?: string}> = () => Promise.resolve({ ok: true, picks: [{ id: 101, type: 'track' }] })
  ...
  const c = new RefillController({
    ...
    getExclusion: () => [9],
    requestBatch: (req: any) => {
      calls.push(req)
      return responder(req)
    },
    onBatch: (picks: string|any[]) => {
      batches.push(picks)
      return picks.length
    },
    onStatus: (text: any, cls: any) => statuses.push({ text, cls }),
  })
...
async function runServerChecks() {
  ...
  const sameBatch = JSON.stringify((a.picks || []).map((p: { id: any }) => p.id)) === JSON.stringify((b.picks || []).map((p: { id: any }) => p.id))
  ...
  const libIds = lib.liked.tracks.map((t: { id: any }) => t.id)
  ...
  const exclIds = (excl.picks || []).map((p: { id: any }) => p.id)
  check('服务端：候选会排除当前曲与已排队曲，且结果不重复', excl.ok && exclIds.every((id: any) => !excludedFirst.includes(id)) && new Set(exclIds).size === exclIds.length, {
    excluded: excludedFirst.length,
    picks: exclIds,
  })
  check('服务端：进队列前每首都有可播性校验', (excl.picks || []).every((p: { playable: boolean; type: string }) => p.playable === true && p.type === 'track'), {
    kinds: (excl.picks || []).map((p: { kind: any }) => p.kind),
  })
  ...
  check('服务端：Codex 超时时用曲库候选降级续播', d1.ok && d1.degraded === true && d1.source === 'library' && (d1.picks || []).length > 0 && (d1.picks || []).every((p: { playable: any }) => p.playable), {
    reason: d1.reason,
    picks: (d1.picks || []).map((p: { id: any }) => p.id),
  })
...
async function runRealCodexRefill() {
  ...
  check('真实 Codex 补歌返回可播批次，并带真实 token 统计', r.ok === true && r.source === 'codex' && (r.picks || []).length > 0 && (r.picks || []).every((p: { playable: any }) => p.playable), {
    wallMs: Date.now() - t0,
    picks: (r.picks || []).map((p: { id: any; name: any; reason: any }) => ({ id: p.id, name: p.name, reason: p.reason })),
...
async function pickPlayableTracks(n: number) {
  const lib = await api('/api/library')
  const out = []
...
async function setSettings(pairs: { [s: string]: unknown }|ArrayLike<unknown>) {
  for (const [key, value] of Object.entries(pairs)) {
    await post('/api/settings', { key, value })
  }
}

async function runBrowserChecks(tracks: any[]) {
  ...
  const replaceQueue = (ids: any[]) =>
    page.evaluate((ts) => window.__radio.__test.replaceQueue(ts), ids.map((id: any) => tracks.find((t: { id: any }) => t.id === id)))
  const seekEnd = async () => {
  ...
  const startPlaying = async (ids: any[]) => {
    await post('/api/session/stop')
  ...
  await page.evaluate((trackId) => {
    const a = document.querySelector('audio')
    const trace: NaturalPlaybackTrace = { trackId, start: null, end: null, seeks: [], rateChanges: [] }
  ...
  report.evidence.naturalContinuation = {
    ...
    playbackSamples: playbackSamples.map((s: { currentId: any; loadedId: any; audioSrc: any; currentTime: any; paused: any; ended: any; readyState: any; resolving: any; status: any }) => ({
  ...
  check('网页：第一首自然播完后自动接入后台补充的一批（没有因队列耗尽停止）', naturalCompleted && isBatchTrackPlaying(playbackSamples[0], playbackSamples[1]), {
    ...
    progress: playbackSamples.map((s: { currentTime: any }) => s.currentTime),
