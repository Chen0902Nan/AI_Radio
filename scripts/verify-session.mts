 ...
 * 用法：node scripts/verify-session.mts [--headful] [--base=http://127.0.0.1:8787]
 */
...
async function runSessionApiChecks(db: { sessions: any; plays?: () => any; feedbackActive?: (trackId: any) => any; feedbackAll?: (trackId: any) => any }) {
  ...
  const ended = rows.filter((r: { ended_at: any; end_reason: string }) => r.ended_at && r.end_reason === 'stopped')
...
async function runFeedbackChecks(db: { sessions?: () => any; plays?: () => any; feedbackActive: any; feedbackAll: any }, track: { id: any; name: any; artists: any }) {
  ...
  check('改主意时旧的生效反馈被替换，同一时刻只有一条生效', dislike.ok && activeRows.length === 1 && activeRows[0].sentiment === 'dislike', {
    activeRows: activeRows.map((r: { sentiment: any }) => r.sentiment),
  ...
  check('撤销是标记而不是删除历史（可回溯）', all.length >= 2 && all.every((r: { revoked_at: any }) => r.revoked_at), {
    rows: all.map((r: { sentiment: any; revoked_at: any }) => ({ sentiment: r.sentiment, revoked: Boolean(r.revoked_at) })),
  })
...
async function sampleIds(n: number, times: number) {
...
async function runWeightingChecks(db: { sessions: () => any; plays: () => any; feedbackActive: (trackId: any) => any; feedbackAll: (trackId: any) => any }, track: { id: any; name: any; artists: any }) {
...
async function runBrowserChecks(db: { sessions: any; plays: any; feedbackActive?: (trackId: any) => any; feedbackAll?: (trackId: any) => any }) {
  ...
  check('播放记录已落库且挂在会话上', playRows.some((r: { session_id: any }) => r.session_id === s1.sessionId), {
  ...
  const sessionsAfterReload = db.sessions().filter((r: { ended_at: null }) => r.ended_at === null)
  ...
  const openRows = db.sessions().filter((r: { ended_at: null }) => r.ended_at === null)
  check('停止后服务端没有残留的进行中会话', openRows.length === 0, { openSessions: openRows.length })

  const outcomes = db.plays().map((r: { outcome: any }) => r.outcome).filter(Boolean)
...
async function main() {
  ...
  const db = {
    ...
    feedbackActive: (trackId: any) =>
      conn.prepare('SELECT * FROM feedback WHERE track_id = ? AND revoked_at IS NULL').all(trackId),
    feedbackAll: (trackId: any) => conn.prepare('SELECT * FROM feedback WHERE track_id = ?').all(trackId),
  }
