 ...
 * 用法：node scripts/read-library.mjs
 */
...
async function main() {
  ...
  for (const pl of targets) {
    try {
      ...
      report.playlistTracks.push({ id: pl.id, name: pl.name, error: redact(err.message) })
      report.steps.push({ step: 'playlistTracks', ok: false, id: pl.id, error: redact(err.message) })
    }
  }
  ...
  try {
    ...
    report.errorHandling = { bogusId: 999999999999, threw: redact(err.message), note: '按预期抛错' }
  }

  // 5. 权限分布抽样（不输出任何播放地址）
  const probe = likedTracks.slice(0, 12)
  const dist = { full: 0, trial: 0, none: 0, error: 0 }
  const details = []
  for (const t of probe) {
    try {
      ...
      dist.error += 1
      details.push({ id: t.id, name: t.name, error: redact(err.message) })
    }
  }
...
main().catch((err) => {
  console.error('读取失败：', redact(err.stack || err.message))
  process.exitCode = 1
})
