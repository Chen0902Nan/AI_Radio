...
test('浏览器：连续歌曲→SSE就绪DJ→歌曲、补歌、暂停恢复与新会话', {timeout:45000},async t=>{
 ...
 const push=data=>{for(const c of clients)c.write(`event: event\ndata: ${JSON.stringify({v:1,seq:++seq,...data})}\n\n`)}
 const server=http.createServer(async(req,res)=>{
  try{
   ...
   const json=data=>{res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify({ok:true,...data}))}
 ...
 const observation=()=>page.$eval('[data-radio-observation]',el=>JSON.parse(el.dataset.radioObservation))
 const click=async label=>{for(const b of await page.$$('button')){if(await b.evaluate(el=>el.textContent.trim())===label){await b.click();return}}throw Error('找不到按钮 '+label)}
 await page.waitForFunction(()=>JSON.parse(document.querySelector('[data-radio-observation]')?.dataset.radioObservation||'{}').queueLength===8)
