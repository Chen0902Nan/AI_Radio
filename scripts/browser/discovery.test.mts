...
test('真实浏览器：首批等待、来源记录、停止不误播、手动歌单后自动接回混合电台',async t=>{
 ...
 const click=async (text: string)=>{for(const b of await page.$$('button'))if((await b.evaluate(el=>el.textContent.trim())).startsWith(text)){await b.click();return}throw Error('missing '+text)}
 ...
 while(!pendingBatch)await new Promise(r=>setTimeout(r,10));(pendingBatch as unknown as () => void)();pendingBatch=null
