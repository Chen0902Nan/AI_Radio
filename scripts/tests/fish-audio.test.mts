...
function audioResponse(buf: Buffer<ArrayBuffer>, extra: Record<string, string> = {}) {
  return {
    ok: true,
    status: 200,
    headers: {
      get: (n: string) => {
        const h: Record<string, string> = { 'content-type': 'audio/mpeg', 'content-length': String(buf.length), ...extra }
...
test('成功合成：显式免费模型请求头 + 正确请求体，产出有效音频与时长', async () => {
  ...
  const fetchImpl = async (url: any, opts: { headers: any; body: string }) => {
    seen.url = url
...
test('免费模型固定不来自配置：省略 model 配置时仍显式发送 s2.1-pro-free', async () => {
  const seen: Record<string, any> = {}
  const fetchImpl = async (url: any, opts: { headers: any }) => {
    seen.headers = opts.headers
...
test('超时有界并取消请求', async () => {
  const fetchImpl = (url: any, opts: { signal: { addEventListener: (arg0: string,arg1: () => void) => void } }) => new Promise((resolve, reject) => {
...
test('错误响应正文迟迟不结束时，超时上限内仍要返回结果（不无限等待）', async () => {
  ...
  const fetchImpl = async (url: any, opts: { signal: { aborted: any; addEventListener: (arg0: string,arg1: () => void,arg2: { once: boolean }) => void } }) => ({
    ok: false,
    status: 429,
    headers: { get: () => null },
    text: () =>
