...
const song = id => ({id,name:'曲'+id,ar:[{name:'歌手'}],al:{name:'专辑'},dt:200000})
function musicFixture(identity='fixture') {
 ...
 user_playlist:async({offset})=>({body:{code:200,playlist:offset?[]:[{id:10,userId:1,trackCount:2}],more:false}}),
 playlist_detail:async()=>({body:{code:200,playlist:{trackCount:2,trackIds:[{id:3},{id:4}]}}}),
 song_detail:async({ids})=>({body:{code:200,songs:String(ids).split(',').map(Number).map(song)}}),
 ...
 song_url_v1:async({id})=>({body:{code:200,data:[{id,url:'https://example.com/'+id,expi:1200}]}}),
 })
...
test('不喜欢为具体版本硬排除，撤销后恢复；喜欢用于相似歌曲种子',async()=>{
 ...
 const seeds=[];upstream.simi_song=async({id})=>{seeds.push(id);return {body:{code:200,songs:[11,12,13,14].map(song)}}}
...
test('音源只有试听或不可用时明确失败，不伪造可播结果',async()=>{
 musicFixture('trial')
 upstream.song_url_v1=async({id})=>({body:{code:200,data:[{id,url:'https://example.com/trial',freeTrialInfo:{end:30}}]}})
 const {session}=await call('session/start',{})
...
test('近期歌曲仅在未播候选无法完整播放时放回并提示',async()=>{
 musicFixture('recent')
 const {automatic}=await call('plays/history'), recentId=automatic.find(p=>p.selection_source==='discovery').track_id
 upstream.song_url_v1=async({id})=>({body:{code:200,data:[{id,url:id===recentId?'https://example.com/full':null}]}})
 const {session}=await call('session/start',{})
...
test('有探索候选但全为试听时，返回歌单内歌曲并明确说明原因',async()=>{
 ...
 upstream.song_url_v1=async({id})=>({body:{code:200,data:[{id,url:'https://example.com/'+id,freeTrialInfo:id>=11?{end:30}:null}]}})
 const {session}=await call('session/start',{})
 const batch=await call('queue/refill',{sessionId:session.id,epoch:1,count:4})
...
test('探索连续超时仍保留歌单内兜底的查询预算',async t=>{
 ...
 upstream.song_url_v1=async({id})=>{if(id>=11){now+=16000;throw Error('timeout')}return {body:{code:200,data:[{id,url:'https://example.com/full'}]}}}
