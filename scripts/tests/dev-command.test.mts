...
test('dev：冷启动先构建，修改后重启，编译失败保留服务，退出清理子进程', { timeout: 20000 }, async t => {
  ...
  fs.writeFileSync(path.join(dir, 'service.mjs'), `
    ...
    setInterval(()=>{},1000);
  `)
  const runner = pathToFileURL(path.resolve('scripts/lib/dev-runner.mts')).href
  ...
  let output = '', exitCode: unknown
  child.stdout.on('data', b => { output += b })
  child.stderr.on('data', b => { output += b })
  const done = new Promise<void>(resolve => child.on('exit', code => { exitCode = code; resolve() }))
  t.after(async () => { if (exitCode === undefined) child.kill('SIGTERM'); await done; moveToTrash(dir) })
  async function waitFor<T>(predicate: () => T | Promise<T>) {
