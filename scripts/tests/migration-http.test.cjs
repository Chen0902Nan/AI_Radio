...
function response(){const r=new Writable({write(chunk,encoding,done){r.body+=chunk;done()}});r.body='';r.headers={};r.status=n=>(r.statusCode=n,r);r.header=(k,v)=>(r.headers[k]=v,r);r.writeHead=(n,h)=>(r.statusCode=n,Object.assign(r.headers,h),r);return r}
...
test('fail-next只令下一次音源解析失败，消费后恢复正常',async()=>{
 ...
 const music=new MusicController({resolveTrack:async id=>({kind:'full',id})})
...
test('强制补歌曲目经过可播性检查且只影响指定批次',async()=>{
 ...
 const ncm={loadSession:()=>({cookie:'fixture'}),currentIdentity:()=> 'forced-fixture',selectionLibrary:async()=>({complete:true,tracks:[{id:11,name:'曲11'},{id:12,name:'曲12'}]}),getLikedIds:async()=>[11,12],getLikedTracks:async()=>[{id:11,name:'曲11'},{id:12,name:'曲12'}]}
 const {OrchestratorService}=require('../../apps/api/dist/preparation/orchestrator.service.js')
 const orchestrator=new OrchestratorService({resolveTrack:async id=>({kind:id===11?'full':'unplayable'})},{},{getNumberSetting:()=>5},{publish(){}})
 ...
 assert.equal(a.statusCode,200);assert.deepEqual(JSON.parse(a.body).picks.map(p=>p.id),[11]);assert.equal(JSON.parse(a.body).source,'forced')
