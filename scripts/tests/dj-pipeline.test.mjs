...
function makeDeps(over = {}) {
  ...
  const deps = {
    ...
    djScript: {
      generateSegueScript: async (input) => {
...
async function completeReady(d, req) {
  const s = contract.SAMPLES.sourcedScript()
...
test('basic_only 短音频允许降级使用并记录时长偏差', async () => {
  ...
  try {
    ...
    const job = p.job(r.job.segueId).job
    assert.equal(job.state, 'ready')
    assert.ok(job.deviations.some((x) => x.code === 'below_target_seconds'), JSON.stringify(job.deviations))
  } finally {
    d.cleanup()
  }
})
