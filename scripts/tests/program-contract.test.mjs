...
function assertRejected(res, code) {
...
test('全部失败样例以预期错误码被拒绝', () => {
  const dispatch = {
    script: (v, opts) => c.validateScript(v, opts),
    prepare: (v) => c.validatePrepareRequest(v),
    job: (v) => c.validateSegueJob(v),
    sources: (v) => c.validateSources(v),
  }
