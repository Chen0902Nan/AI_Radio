...
更新日期：2026-09-18（调整默认选歌比例）。

> **最新状态及证据指针（2026-09-17 补记，规格正文保留原样）**：「尚未进入实施」针对的是本规格的全部范围。音乐链路（登录/播放/Codex 选歌/自动补歌/会话反馈/DJ 串场）已实施并通过离线与真实链路验证，进度与证据见 [.scratch/radio-agent/verification/report.md](verification/report.md) 与 [.scratch/dj-segue/verification/access.md](../dj-segue/verification/access.md)。天气、日历、开场播报、聊天、每日准备仍是未实现项，以规格正文为准。
...
- 默认约 50% 选自红心、收藏歌单或自建歌单，50% 从歌单外按口味探索。比例是默认目标，不要求每一小段严格配比。
- 按最近 50 首实际开始播放的自动歌曲滚动统计比例，跨次收听保留；手动歌曲与 DJ 不计入，更早的偏差不再影响选歌。详见 [探索选歌访谈 Q7、Q9](../discovery-selection/interview.md)。探索选歌已实现，本地与真实只读核验见 [验收记录](../discovery-selection/verification.md)。
- 自动选歌尽量避开最近 50 首实际开始播放的自动歌曲，跨次收听生效；可播候选不足时允许提前重复并提示，明确点歌不受限制。近期防重复只针对听过的具体版本，同一歌手同一首歌的现场版、重录版或另一专辑版本仍可推荐；以上由探索选歌访谈 Q10、Q11 确认。
- 探索歌曲不等于用户从未听过，也不特指近期发行。
- 下一首立即跳过当前歌曲，但不改变长期音乐口味。只有明确点击喜欢／不喜欢才调整长期偏好，反馈保存在本地。
- 标记“不喜欢”的具体版本不再自动推荐，直到撤销；候选不足也不自动放回，但仍允许明确手动点播，不连带排除其他版本或歌手。该明确排除优先于候选不足时允许重复的规则（探索选歌访谈 Q12）。
- 标记“喜欢”主要用于寻找相似、合口味的探索歌曲，不额外增加原歌的重播机会；原歌仍按普通选歌和近期防重复规则安排，不改变歌单内外分类（探索选歌访谈 Q13）。
- 歌单资料读取不完整且没有可用的完整记录时，暂缓新增探索，先播放已确认的可播歌单内歌曲并提示原因，读取恢复后继续探索；缺失资料不能视为空歌单（探索选歌访谈 Q14）。本轮完整开发范围见 [探索选歌开发规格](../discovery-selection/spec.md)。
