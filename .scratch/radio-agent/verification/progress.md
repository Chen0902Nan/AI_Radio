...
## 自动补歌与统一节目编排（2026-09-16）

本次范围：只做「自动补歌 + 统一节目编排」，对应规格「及时响应／选歌执行／异常恢复」中尚未落地的部分。DJ/TTS、天气、飞书日历、聊天、每日调度、70/30 探索仍未实现。

- **编排层**：新增 `server/orchestrator.js`（候选抽样、可播性校验、补歌协调）与 `public/orchestrator.js`（客户端补歌控制器）。播放器仍是原来那一个 `<audio>`，没有第二套播放实现。
- **自动补歌**：待播 ≤2 首时后台准备下一批（默认 5 首），只追加、不替换、不打断当前歌曲；同会话同一意图不会并发生成多批。
- **失败处理**：Codex 超时/额度/输出无效时，降级用已取得的曲库候选续播（仍逐首校验可播），并给该会话指数冷却（1→15 分钟）；冷却期内不再消耗订阅。
- **有界恢复**：候选耗尽/无可播/音乐不可用时，客户端 30s→5min、最多 5 次退避，之后进入等待恢复，需用户动作才再试。
- **意图边界**：暂停不作废后台结果但绝不自动出声；停止/换来源/应用新一批会让旧任务作废；刷新只重连、不新建会话、不自动出声。
- **候选去重**：排除当前曲、已待播曲、已失败曲；延续喜欢/不喜欢加权与近期播放降权。
- **接口留白**：队列条目带 `type`，非 track 类型明确跳过，为后续播报留位置但不写成已完成。

验证（独立数据库 + 独立服务进程，不污染真实数据）：

- `npm run verify:orchestration`：第一轮 33 通过；复核修复后 **40 通过 / 0 失败**（控制器单元 13、服务端协调 11、真实 Codex 1、真实浏览器 15，含 198.6s 自然播完的跨批续播）。证据：`artifacts/orchestration.json`。
- 回归：`read-library` 退出码 0（failures 为空）、`verify:playback` 14/14、`verify:fixes` 18/18、`verify:session` 25/25、`verify:codex --skip-real` 9/9。

复核修复（详见报告第 10.6 节）：

- 默认曲库播到底仍会停止：前端按整个历史队列去重，服务端返回的已播曲目被全部过滤成 0 首。已统一去重范围（当前＋待播＋已失败），并把实际追加 0 首改为失败退避。
- 迟到的旧请求会取消新请求：服务端不比 epoch 大小。已改为拒绝过期请求、不影响在途新任务；客户端 epoch 跨刷新单调，并在发送前丢弃本地已取消的请求。

未验证：两小时连续收听（除首曲自然播完外，其余边界仍用拖动加速）、真实限流/额度耗尽、试听片段真实样本、多标签页并发。

详见 [report.md 第 10 节](report.md)。
