...
const {setup,flush,item,deferred}=require('./support/runtime.cjs')
...
test('坏歌的延迟重试不能覆盖用户暂停或手动选曲',async t=>{
 t.mock.timers.enable({apis:['setTimeout']})
 const {runtime:r,audio}=setup(t,[1,2,3,4,5],{resolve:async id=>id===1?{ok:true,playable:false,code:'unplayable'}:{ok:true,playable:true,audioUrl:'/api/audio/'+id}})
...
test('SSE首次连接和重连读取配置快照，但不自动开播',async t=>{
 const saved=global.EventSource,streams=[]
 global.EventSource=class { constructor(url){this.url=url;this.listeners={};streams.push(this)} addEventListener(n,f){this.listeners[n]=f} close(){this.closed=true} emit(n,data){this.listeners[n]?.({data:JSON.stringify(data)})} }
...
test('SSE配置事件过滤旧会话与重复序号，合法通知刷新快照',async t=>{
 const saved=global.EventSource,streams=[]
 global.EventSource=class {constructor(){this.listeners={};streams.push(this)}addEventListener(n,f){this.listeners[n]=f}close(){}emit(n,data){this.listeners[n]?.({data:JSON.stringify(data)})}}
 t.after(()=>{global.EventSource=saved})
 let voice='a'
...
test('DJ就绪推送立即查任务并接管串场，连接正常时不周期轮询',async t=>{
 t.mock.timers.enable({apis:['setTimeout']})
 const saved=global.EventSource,streams=[]
 global.EventSource=class {constructor(){this.listeners={};streams.push(this)}addEventListener(n,f){this.listeners[n]=f}close(){}emit(n,data){this.listeners[n]?.({data:JSON.stringify(data)})}}
 t.after(()=>{global.EventSource=saved})
 let request,jobQueries=0,ready=false
