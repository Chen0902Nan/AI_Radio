...
for (const ext of ['.ts', '.tsx']) require.extensions[ext] = (mod, file) => (mod as NodeModule & { _compile(source: string, filename: string): void })._compile(ts.transpileModule(fs.readFileSync(file, 'utf8'), { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.CommonJS, jsx: ts.JsxEmit.ReactJSX, esModuleInterop: true } }).outputText, file)
const { makeTrackItem } = require('@radio/contracts')
exports.item = (id: any) => makeTrackItem({ id, name: `曲${id}`, artists: '歌手', durationMs: 200000 })
...
exports.setup = (t: { after: (arg0: () => void) => void }, ids=[101,102], overrides={}) => {
 const listeners = new Map()
 const audio = { paused:true,currentTime:0,duration:200,readyState:4,src:'',currentSrc:'',error:null,
 addEventListener(n: any,f: any){if(!listeners.has(n))listeners.set(n,new Set());listeners.get(n).add(f)},removeEventListener(n: any,f: any){listeners.get(n)?.delete(f)},
 removeAttribute(){this.src='';this.currentSrc=''},play(){this.paused=false;return Promise.resolve()},pause(){this.paused=true},
 emit(n: string){if(n==='playing')this.currentSrc=this.src;if(n==='ended')this.paused=true;for(const f of listeners.get(n)||[])f()} }
 const {api}=require('../../src/api/client.ts')
 const saved={...api}, savedDocument=global.document
 const records: any[]=[]; let session=1
 Object.assign(api,{
 resolve:async (id: string)=>({ok:true,playable:true,audioUrl:'/api/audio/'+id}),
 ...
 playStart:async (data: any)=>{const row={id:records.length+1,...data,outcome:null};records.push(row);return{ok:true,playId:row.id,session:{id:'s'+session}}},
 playEnd:async(id: any,outcome: any)=>{records.find(r=>r.id===id).outcome=outcome;return{ok:true}},
